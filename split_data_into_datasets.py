...
for cls in classes_dir:
    ...
    src_folder = root_dir + cls  # Folder to copy images from

    allFileNames = os.listdir(src_folder)
    np.random.shuffle(allFileNames)
    train_FileNames, val_FileNames, test_FileNames = np.split(np.array(allFileNames),
                                                              [int(len(allFileNames) * train_ratio),
                                                               int(len(allFileNames) * (train_ratio + val_ratio))])
    ...
    for name in train_FileNames:
        dst = os.path.join(root_dir, 'train/', cls)
        shutil.copy(name, dst)

    for name in val_FileNames:
        dst = os.path.join(root_dir, 'val/', cls)
        shutil.copy(name, dst)

    for name in test_FileNames:
        dst = os.path.join(root_dir, 'test/', cls)
        shutil.copy(name, dst)
